...
def compute_and_plot(q_deg, Ks_rot, Ks_aa, Ks_ax, Ke):
    ...
    axes_lengths = np.sqrt(eigvals) #is sqrt needed?
...
Ke_slider = Slider(slider_axes[4], 'Elbow Stiffness', 0.1, 200, valinit=5) #units for stiffness are Nm/rad
Ks_ax_slider = Slider(slider_axes[5], 'Shoulder Axial', 0.1, 500, valinit=5)
Ks_aa_slider = Slider(slider_axes[6], 'Shoulder AA', 0.1, 500, valinit=5)
Ks_rot_slider = Slider(slider_axes[7], 'Shoulder Rot', 0.1, 500, valinit=5)
